...
namespace lab2
{
    class Phone : Device, ISMS, IRinging, IDownload
    {
        ...
        public string Message
        {
            set
            {
                if (value.Length > 0 && value.Length <= 30) message = value;
                else throw new OverLengthError(new ExceptionArgs(value, 30));
            }
            get
            {
                return message;
        ...
        void ISMS.notification()
        {
            Console.WriteLine($"Notification from SMS");
        }
